...
###########################################################################################################################
    def __init__(
        ...
        for (year, name) in image_sets:
            self._year = year
            rootpath = '/home/ec2-user/workspace_ahmad/YOLOX/datasets/custom_dataset/dataset'

            for path in os.listdir(rootpath):
                self.ids.append((rootpath, path))


        #Code to integrate output.manifest into the code self.annotations = bounding_boxes
        res = np.empty((0, 5))
        ...
        res = np.empty((0, 5))  # Create an res list that will contain normalized bounding boxes in an image
        bounding_boxes = [] # Create a bounding_boxes list that is a list of all res lists
        ...
        for line in data_list:
            ...
            img_size = (640, 640)
            r = min(img_size[0] / im_height, img_size[1] / im_width)
            
            if not annotations:
                res = np.empty((0, 5))
                res[:, :4] *= r  #scare input height and width with image height and width
                resized_info = (int(im_height * r), int(im_width * r))
                bounding_boxes.append((res, img_info, resized_info)) 
        #         bounding_boxes.append((res, img_info, resized_info)) 
            else:
                for line in annotations:
                    ...
                    dimensions = [xmin, ymin, xmax, ymax, class_id]
                ...
                if np.size(res):   
                    ...
                    resized_info = (int(im_height * r), int(im_width * r))
                    bounding_boxes.append((res, img_info, resized_info))  
                res = np.empty((0, 5)) # resetting the list to contain bounding boxes of next image
 ...
 #############################################################################################################################           
    ...
    def evaluate_detections(self, all_boxes, output_dir=None):
        ...
        """
        self._write_voc_results_file(all_boxes) #This writes our detections on eval dataset to one file/class
        IouTh = np.linspace(
